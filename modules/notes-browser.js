/* =========================================================
   ...
   Browse, view, and interact with OSM Notes via proxy API.
   ========================================================= */
...
function renderNotesUI(container) {
  ...
  container.innerHTML =
    ...
    (isEl ? 'Φόρτωση' : 'Load Notes') + '</button>' +
  ...
  try {
    ...
    if (statsEl) {
      statsEl.textContent = isEl
        ? 'Βρέθηκαν ' + filtered.length + ' σημειώσεις'
        : 'Found ' + filtered.length + ' notes';
    }
...
function parseNotesXML(xmlText) {
  ...
  noteElems.forEach(function (elem) {
    var note = {
      ...
      date: elem.getAttribute('date_created') || elem.getAttribute('date') || new Date().toISOString(),
      comments: [],
      comment: null,
...
function showNoteDetails(note) {
  ...
  var commentsHtml = note.comments.map(function (c) {
    return '<div class="note-description">' +
      ...
      (c.action ? '<br/><span style="color:#6d4aff">' +
      escapeHtml(c.action) + '</span>' : '') +
  ...
  content.innerHTML =
    ...
    (isEl ? 'Συντεταγμένες:' : 'Coords:') + '</strong> ' +
    note.lat.toFixed(6) + ', ' + note.lon.toFixed(6) + '</div>' +
    '  <div style="margin-bottom:0.5rem;"><strong>🔄 ' +
    (isEl ? 'Κατάσταση:' : 'Status:') + '</strong> ' +
    (note.status === 'open'
      ? '🔵 ' + (isEl ? 'Ανοικτή' : 'Open')
      : '✅ ' + (isEl ? 'Κλειστή' : 'Closed')) +
    '</div>' +
    '  <h4>💬 ' + (isEl ? 'Σχόλια' : 'Comments') +
    ' (' + note.comments.length + ')</h4>' +
    commentsHtml +
    '  <a href="' + note.url + '" target="_blank" ' +
    'class="btn btn-sm" style="margin-top:0.5rem;">' +
    (isEl ? 'Άνοιγμα στο OSM' : 'Open in OSM') + '</a>' +
    '</div>';
